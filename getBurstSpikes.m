...
%  - save spike places for every 10ms into one frame
%  - output burst#.mat file for each burst
...
% extra bins before burst starts. The value of each pixel represent spike
% count of the same neuron (pixel)
% (NOTE: bin size = 10ms, time step size = 0.1 ms)
% ------------------------------------------------------------------------
for i = 1:numBurst
    % get burst info from 10 bins before burst
    frames = zeros(numNeurons, b(i,3)+15, 'int8');
    s_time = (b(i,1)-10) * 100;         % start bin# -> start time step
    e_time = (b(i,2)+5) * 100;              % end bin# -> end time step  
        ...
        if isempty(start) == 0
            for k = start:ended
                if ((P(k,j) >= s_time) && (P(k,j) <= e_time))
                    % aggregate 10 ms (100 time step) into one frame
